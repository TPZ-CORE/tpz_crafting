...
local LoadItemsList = function ()
    TriggerEvent("tpz_core:ExecuteServerCallBack", "tpz_crafting:getItemsList", function(data)
        ClientData.ItemsList = data
    end)
end
...
RegisterNetEvent('tpz_crafting:onCraftingBookUse')
AddEventHandler('tpz_crafting:onCraftingBookUse', function(item, itemId)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if ClientData.HasCooldown then
            TriggerEvent('tpz_inventory:closePlayerInventory')
        end

        if not isDead and not ClientData.HasCraftingOpen and ClientData.Loaded and not ClientData.HasCooldown then

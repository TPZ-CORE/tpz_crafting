...
PickupPlacedObject = function(objectEntity, model, recipe)
    ...
    TriggerEvent("tpz_core:ExecuteServerCallBack", "tpz_crafting:startPlacedObjectPickupAction", function(cb)
        ...
        if not cb then
            TriggerEvent('tpz_core:sendRightTipNotification', "~e~" .. Locales['NOT_ENOUGH_INVENTORY_SPACE_PICKUP'], 4000)
            ClientData.HasCooldown = false
            return
        end
        ...
        TriggerServerEvent("tpz_crafting:pickupPlacedObjectIngredients", recipe.Ingredients)
...
if not Config.exp_target_menu.enabled then
    ...
    Citizen.CreateThread(function ()
        ...
        while true do
            ...
            if not isDead and ClientData.Loaded and not ClientData.HasCooldown and not ClientData.HasCraftingOpen then
